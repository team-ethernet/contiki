...
UNIT_TEST(json_empty_string){
  ...
  INIT_SENML_JSON();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(json_noise_sensor){
  ...
  INIT_SENML_JSON();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:dev:mac:fcc23d0000003790", UNIT, "dB", BOOLEAN_VALUE, 0, VALUE, 50.00);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(json_many_parameters){
  ...
  INIT_SENML_JSON();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:dev:mac:fcc23d0000003790", BASE_TIME, 123456789.00, BASE_UNIT, "Volt", BASE_VERSION, 2, VALUE, -1.00, SUM, 0.00, TIME, 643.00, UPDATE_TIME, 0.00);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(json_multiple_records){
  ...
  INIT_SENML_JSON();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:dev:mac:fcc23d0000003790", UNIT, "dB", VALUE, 50.00);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:dev:mac:fcc23d0000003788", UNIT, "w", VALUE, 0.00);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:dev:mac:fcc23d0000013290", UNIT, "m", VALUE, 35.25);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(cbor_noise_sensor){
  ...
  INIT_SENML_CBOR();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:mac:fcc2948375028573", UNIT, "dB", VALUE, 59.23);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(cbor_multiple_records){
  ...
  INIT_SENML_CBOR();
  len += START_SENML_PACK_STREAM(buffer_pointer, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_NAME, "urn:mac:fcc2948375028573", UNIT, "dB", VALUE, 8923.223);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BOOLEAN_VALUE, 0, BASE_TIME, 123456789.00);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
...
UNIT_TEST(cbor_many_parameters) {
  ...
  INIT_SENML_CBOR();
  len += START_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
  len += ADD_RECORD(buffer_pointer + len, BUFFER_SIZE - len, BASE_VERSION, 5, UNIT, "m/s", VALUE, 12.608765);
  len += END_SENML_PACK_STREAM(buffer_pointer + len, BUFFER_SIZE - len);
