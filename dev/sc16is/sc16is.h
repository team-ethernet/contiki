...
#define SC16IS_RHR	  (0x00) /* RX FIFO */
#define SC16IS_THR	  (0x00) /* TX FIFO */
#define SC16IS_IER	  (0x01) /* Interrupt enable */
#define SC16IS_IIR	  (0x02) /* Interrupt Identification */
#define SC16IS_FCR	  (0x02) /* FIFO control */
#define SC16IS_LCR	  (0x03) /* Line Control */
#define SC16IS_MCR	  (0x04) /* Modem Control */
#define SC16IS_LSR	  (0x05) /* Line Status */
#define SC16IS_MSR	  (0x06) /* Modem Status */
#define SC16IS_SPR	  (0x07) /* Scratch Pad */
#define SC16IS_TXLVL	  (0x08) /* TX FIFO level */
#define SC16IS_RXLVL	  (0x09) /* RX FIFO level */
#define SC16IS_IODIR	  (0x0a) /* I/O Direction - only on 75x/76x */
#define SC16IS_IOSTATE	  (0x0b) /* I/O State - only on 75x/76x */
#define SC16IS_IOINTENA	  (0x0c) /* I/O Interrupt Enable  - only on 75x/76x */
#define SC16IS_IOCONTROL  (0x0e) /* I/O Control  - only on 75x/76x */
#define SC16IS_EFCR	  (0x0f) /* Extra Features Control */

/* TCR/TLR Register set: Only if ((MCR[2] == 1) && (EFR[4] == 1)) */
#define SC16IS_TCR	  (0x06) /* Transmit control */
#define SC16IS_TLR	  (0x07) /* Trigger level */

/* Special Register set: Only if ((LCR[7] == 1) && (LCR != 0xBF)) */
#define SC16IS_DLL	  (0x00) /* Divisor Latch Low */
#define SC16IS_DLH	  (0x01) /* Divisor Latch High */

/* Enhanced Register set: Only if (LCR == 0xBF) */
#define SC16IS_EFR	  (0x02) /* Enhanced Features */
#define SC16IS_XON1	  (0x04) /* Xon1 word */
#define SC16IS_XON2	  (0x05) /* Xon2 word */
#define SC16IS_XOFF1	  (0x06) /* Xoff1 word */
#define SC16IS_XOFF2	  (0x07) /* Xoff2 word */
...
/* EFCR register bits */
#define SC16IS_EFCR_9BIT_MODE_BIT	(1 << 0) /* Enable 9-bit or Multidrop
						  * mode (RS485) */
...
#define SC16IS_EFCR_IRDA_MODE_BIT	(1 << 7) /* IrDA mode
...
#define SC16IS_EFR_ENABLE_BIT	(1 << 4) /* Enable enhanced functions
